...
export default function ChatPanel({
  ...
  {documents.length > 1 && (
            <div className="px-4 py-2 border-b border-border flex flex-wrap items-center gap-1.5">
              <span className="text-[11px] font-medium text-subtle mr-1">Scope:</span>
   {documents.map((d) => {
                 ...
                 return (
                    ...
                    className={`text-[11px] rounded-full border px-2 py-0.5 transition-colors ${
                                        checked
                                          ...
                                          : "border-border text-subtle hover:border-accent/40"
                    }`}
                   title={d.name}
                >
      ...
      {disabled
                     ? "Upload a document to start chatting."
                      : documents.length > 1
              ? "Ask a question, or cross-reference any of the documents selected above -- e.g. \"compare operating expenses across these.\""

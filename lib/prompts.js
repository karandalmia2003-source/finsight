// Every system prompt below asks Claude to return raw JSON that gets parsed
// with JSON.parse. The most common way that breaks is Claude quoting a word
// or phrase inside a string value with literal double quotes (e.g. a
// sentence that mentions "rising labor costs"), which prematurely closes
// the JSON string and corrupts the payload. Appending this rule to every
// JSON-producing prompt prevents that failure mode at the source.
export const JSON_SAFETY_RULE = `Critical formatting rule: your entire reply is parsed with JSON.parse, so it must be syntactically valid JSON. If you need to quote or set off a word or phrase inside any string value, use single quotes ('like this') instead of double quotes — never place an unescaped double-quote character inside a JSON string value. Do not use literal newline characters inside string values; use \\n if a line break is needed.`;
...
export function buildChatSystemPrompt(documents) {
  ...
  return `You are the FinSight research assistant. Answer using the financial documents currently in scope for this conversation (provided below as JSON) as your primary source of truth — never let a web result contradict a number actually stated in these documents. The user may have selected a subset of their uploaded documents to focus this conversation on; treat exactly the documents listed below as the full set available to you, and feel free to cross-reference any of them against any others (e.g. trends across periods, company-to-company comparisons) whenever it answers the question. You also have a web_search tool: use it to add current market context, recent news, analyst sentiment, or any other real-world information the documents themselves don't contain, whenever it would make your answer more useful. If a question can't be answered from the documents or from search, say so plainly instead of guessing.
...
DOCUMENTS (in scope for this conversation, JSON):
${JSON.stringify(compact)}

${JSON_SAFETY_RULE}`;
}

export const COMPARE_SYSTEM = `You are a financial analyst writing brief commentary for a side-by-side comparison of two or more financial documents already uploaded to FinSight. You will receive each document's extracted financial JSON, plus a pre-computed table of metric values (and period-over-period deltas) across all of them, in the same left-to-right order the user selected. Do not recompute or restate the numbers yourself — the table is already correct. Your job is to explain, in exactly one sentence per metric, what most plausibly drove the change across the documents — when there are more than two, describe the overall trend across all of them, not just the first-to-last gap — drawing first on the documents' own context (revenue mix shifts, expense categories, risks, trends) and only reaching for outside knowledge you're confident about when the documents alone don't explain it.
